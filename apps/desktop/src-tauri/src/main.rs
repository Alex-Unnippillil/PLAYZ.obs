...
mod commands;
use playz_core::{Core,contracts::{Phase,Settings}};
use std::{collections::HashSet,sync::{Arc,Mutex,atomic::{AtomicBool,Ordering}}};
...
struct DesktopState {quit_requested:AtomicBool,pressed:Mutex<HashSet<u32>>,shortcut_error:Mutex<Option<String>>,settings_gate:tokio::sync::Mutex<()>}
fn show_window(app:&tauri::AppHandle){if let Some(window)=app.get_webview_window("main"){let _=window.show();let _=window.unminimize();let _=window.set_focus();}}
fn notify(app:&tauri::AppHandle,message:&str){let _=app.emit("playz:notice",message);let _=app.notification().builder().title("PLAYZ").body(message).show();}
fn toggle_recording(app:tauri::AppHandle){
    let Some(core)=app.try_state::<Arc<Core>>().map(|s|s.inner().clone())else{return;};
    tauri::async_runtime::spawn(async move{let result=if matches!(core.snapshot().phase,Phase::Recording|Phase::Preparing){core.stop().await}else{core.start(uuid::Uuid::new_v4().to_string()).await};if let Err(e)=result{notify(&app,&e.to_string());}let _=app.emit("playz:changed",());});
}
fn bookmark(app:tauri::AppHandle){let Some(core)=app.try_state::<Arc<Core>>().map(|s|s.inner().clone())else{return;};tauri::async_runtime::spawn(async move{match core.live_bookmark().await{Ok(_)=>notify(&app,"Bookmark saved locally"),Err(e)=>notify(&app,&e.to_string())}let _=app.emit("playz:changed",());});}
fn register_shortcuts(app:&tauri::AppHandle,settings:&Settings)->Result<(),String>{
    let record:Shortcut=settings.hotkey_record.parse().map_err(|_|"Invalid recording shortcut")?;
    let mark:Shortcut=settings.hotkey_bookmark.parse().map_err(|_|"Invalid bookmark shortcut")?;
    app.global_shortcut().unregister_all().map_err(|e|e.to_string())?;
    app.state::<DesktopState>().pressed.lock().unwrap_or_else(std::sync::PoisonError::into_inner).clear();
    app.global_shortcut().register(record).map_err(|_|"Recording shortcut is already used by another application".to_string())?;
    if app.global_shortcut().register(mark).is_err(){let _=app.global_shortcut().unregister_all();return Err("Bookmark shortcut is already used by another application".into());}
    Ok(())
}
fn finish_quit(app:tauri::AppHandle){
    let core=app.state::<Arc<Core>>().inner().clone();
    tauri::async_runtime::spawn(async move{
        if let Err(e)=core.shutdown().await{let message=format!("PLAYZ stopped with a recovery warning: {e}\nAny original media has been preserved. Reopen PLAYZ and use Recover.");app.dialog().message(message).title("PLAYZ recovery notice").show(move |_|app.exit(0));}
        else{app.exit(0);}
    });
}
fn request_quit(app:tauri::AppHandle){
    if app.state::<DesktopState>().quit_requested.swap(true,Ordering::AcqRel){return;}
    if app.state::<Arc<Core>>().is_busy(){let target=app.clone();app.dialog().message("Stop and finalize the recording, then quit PLAYZ? This may take a moment. Your original recording will be preserved.").title("Finish recording before quitting").buttons(MessageDialogButtons::OkCancelCustom("Stop and quit".into(),"Keep recording".into())).show(move |yes|{if yes{finish_quit(target);}else{target.state::<DesktopState>().quit_requested.store(false,Ordering::Release);}});}
    else{finish_quit(app);}
}
fn main(){
    ...
    app.run(|app,event|{if let tauri::RunEvent::ExitRequested{api,code,..}=event{if code.is_none(){api.prevent_exit();request_quit(app.clone());}}});
}
