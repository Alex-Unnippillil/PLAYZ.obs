  ...
  return <div className="app-shell polished-shell">
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom">
        <Button className="quick-launch" aria-label="Quick actions" onClick={() => { setQuickSearch(''); setQuickOpen(true); }}><Search size={16} aria-hidden="true"/>Quick actions<kbd aria-hidden="true">Ctrl K</kbd></Button>
    ...
    <div className="workspace-shell">
      <header className="capture-bar">
        ...
        <div className="row">
          ...
          {!configured && !busy ? <Button variant="primary" disabled={!snapshot || settingsPending} onClick={() => navigate({ view: 'settings' })}>Set up recording<ArrowRight size={16} aria-hidden="true"/></Button> : <Button variant={active ? 'danger' : 'primary'} busy={action.pending} disabled={!snapshot || (busy && !active) || (!active && (settingsPending || !!state.error))} title={snapshot?.settings.hotkey_record} onClick={() => void action.run(active ? api.stop : () => api.start(crypto.randomUUID()))}>{active ? <Square size={15} fill="currentColor" aria-hidden="true"/> : <Circle size={15} fill="currentColor" aria-hidden="true"/>}{active ? 'Stop recording' : snapshot?.phase === 'preparing' ? 'Starting…' : busy ? 'Finalizing…' : 'Record'}</Button>}
        </div>
      </header>
    ...
    <Modal open={quickOpen} onClose={() => setQuickOpen(false)} title="Quick actions" description="Jump to a workspace. Recording shortcuts are managed by the desktop application.">
      <label className="quick-search">Find a page or action<input data-initial-focus value={quickSearch} onChange={e => setQuickSearch(e.target.value)} placeholder="Library, capture, backup…"/></label>
