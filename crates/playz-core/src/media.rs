...
impl Probe {
    pub fn compatible(&self) -> bool {
        self.video_codec == "h264" && self.audio_codec.as_deref().is_none_or(|a| a == "aac")
    }
}
#[derive(Clone)]
pub struct Media {
...
impl Media {
    pub fn new(runtime: &Path) -> Self {
        Self {
            ffmpeg: runtime.join("media/ffmpeg.exe"),
            ffprobe: runtime.join("media/ffprobe.exe"),
        }
    }
    pub async fn probe(&self, path: &Path) -> Result<Probe> {
        ...
        let stderr = process::discard_stderr(
            child
                .stderr
                .take()
                .ok_or("Probe diagnostic stream is missing")?,
        );
        let output = child
            .stdout
            .take()
            .ok_or("Probe response stream is missing")?;
        ...
        match result {
            ...
            Err(_) => {
                ...
                Err("Media inspection timed out; no original file was modified".into())
            }
        }
    }
    pub async fn run(&self, args: Vec<OsString>, control: Control) -> Result<()> {
        ...
        let stderr = process::discard_stderr(
            child
                .stderr
                .take()
                .ok_or("Media diagnostic stream is missing")?,
        );
        let output = child
            .stdout
            .take()
            .ok_or("Media progress stream is missing")?;
        ...
        let reader = tokio::spawn(async move {
            ...
            loop {
                ...
                if line.len() > 8192 {
                    return Err(Error::Message("Media progress line exceeded 8 KiB".into()));
                }
                if let Some(value) = String::from_utf8_lossy(&line)
                    .trim()
                    .strip_prefix("out_time_us=")
                {
                    if let Ok(us) = value.parse::<u64>() {
                        progress.store(us / 1000, Ordering::Relaxed);
                    }
                }
            }
            Ok::<(), Error>(())
        ...
        let result = loop {
            ...
            if control.pause.load(Ordering::Relaxed) {
                let _ = child.kill().await;
                break Err(Error::Paused);
            }
            if Instant::now() > deadline {
                let _ = child.kill().await;
                break Err(Error::Message("Media job exceeded its deadline".into()));
            }
            if let Some(status) = child.try_wait()? {
                break if status.success() {
                    ...
                    Err(Error::Message(format!(
                        "Media processing failed (exit {}). The original was not changed.",
        ...
        reader
            .await
            .map_err(|_| Error::Message("Media progress reader failed".into()))??;
        result
    }
    pub async fn remux(&self, master: &Path, output: &Path) -> Result<Probe> {
        let source = self.probe(master).await?;
        if !source.compatible() {
            return Err("This profile requires H.264 video and AAC audio for offline playback. Original preserved; unsupported conversion was not silently applied.".into());
        }
        if output.exists() {
            let existing = self.probe(output).await?;
            if (existing.duration_ms - source.duration_ms).abs() < 1000.0 {
                return Ok(existing);
            }
            return Err("Existing playback asset does not match its master. Preserve it and inspect the recording folder.".into());
        }
        ...
        if result.is_err() {
            ...
            return result.and(Ok(source));
        }
        let probe = self.probe(&temporary).await?;
        if !probe.compatible() || (probe.duration_ms - source.duration_ms).abs() > 1000.0 {
            return Err("Remux validation failed; the original master remains intact".into());
        }
...
fn input_arguments(master: &Path) -> Vec<OsString> {
    let mut a = strings(&[
        "-protocol_whitelist",
        "file,pipe",
        "-format_whitelist",
    ...
    a.push(master.as_os_str().to_owned());
    a
}
pub fn remux_arguments(master: &Path, temporary: &Path) -> Vec<OsString> {
    let mut a = input_arguments(master);
    a.extend(strings(&[
        "-map",
        "0:v:0",
        "-map",
    ...
    a.push(temporary.as_os_str().to_owned());
    a
}
pub fn export_arguments(master: &Path, temporary: &Path, r: &ExportRequest) -> Vec<OsString> {
    ...
    a.extend(strings(&[
        "-t",
        &format!("{:.6}", (r.end_ms - r.start_ms) / 1000.0),
        "-map",
        "0:v:0",
        "-map",
        "0:a:0?",
    ...
    match r.mode {
        ExportMode::Accurate => a.extend(strings(&[
            "-c:v", "libx264", "-preset", "fast", "-crf", "20", "-pix_fmt", "yuv420p", "-threads",
            "2", "-c:a", "aac", "-b:a", "192k", "-ar", "48000", "-ac", "2",
        ])),
        ExportMode::Fast => a.extend(strings(&["-c", "copy", "-avoid_negative_ts", "make_zero"])),
    }
    a.extend(strings(&["-movflags", "+faststart", "-f", "mp4"]));
    a.push(temporary.as_os_str().to_owned());
    a
}
fn validate_export(probe: &Probe, r: &ExportRequest) -> Result<()> {
    let tolerance = match r.mode {
        ExportMode::Accurate => 350.0,
        ExportMode::Fast => 5000.0,
    };
    if !probe.compatible() || (probe.duration_ms - (r.end_ms - r.start_ms)).abs() > tolerance {
        return Err("Export output did not pass codec/duration validation. Temporary file was preserved for diagnosis.".into());
    }
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn accurate_uses_reencode_and_no_shell() {
        let a = export_arguments(
            Path::new("C:/Videos/my game.mkv"),
            Path::new("C:/Videos/out.mp4"),
            &request(),
        );
        assert!(a.contains(&"libx264".into()));
        assert!(a.contains(&"file,pipe".into()));
        assert!(a.contains(&"C:/Videos/my game.mkv".into()));
        assert!(!a.contains(&"-y".into()));
    }
    #[test]
    fn fast_is_explicit_copy() {
        let mut r = request();
        r.mode = ExportMode::Fast;
        let a = export_arguments(Path::new("x.mkv"), Path::new("y.mp4"), &r);
