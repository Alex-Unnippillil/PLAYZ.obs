...
pub fn local_path(path: &Path) -> Result<PathBuf> {
    ...
    #[cfg(windows)]
    {
        ...
        {
            return Err(
                "Network shares and device paths are not supported for the live library".into(),
            );
        }
    }
    ...
    for part in path.components() {
        ...
        if let Ok(metadata) = fs::symlink_metadata(&current) {
            if metadata.file_type().is_symlink() {
                return Err("Symbolic links are not supported for managed recording paths".into());
            }
            #[cfg(windows)]
            {
                use std::os::windows::fs::MetadataExt;
                if metadata.file_attributes() & 0x400 != 0 {
                    return Err("Choose a local folder without junctions, cloud placeholders or reparse points".into());
                }
            }
        }
    }
...
pub fn existing_media(path: &Path) -> Result<PathBuf> {
    local_path(path)?;
    if !path.is_file() {
        return Err("This media file is missing. Reconnect the drive or use Relink.".into());
    }
    if !matches!(
        path.extension()
            .and_then(|s| s.to_str())
...
pub fn validate_id(id: &str) -> Result<()> {
    uuid::Uuid::parse_str(id)
        .map(|_| ())
        .map_err(|_| Error::Message("Invalid local identifier".into()))
}
...
pub fn reserve_bytes(reserve_gib: u32, bitrate_kbps: u32) -> u64 {
    ...
    (u64::from(reserve_gib) * 1024 * 1024 * 1024)
        .max((u64::from(bitrate_kbps) + 192) * 1000 / 8 * 120)
}
...
mod tests {
    ...
    fn manifest_replacement_is_complete() {
        ...
        atomic_json(&p, &serde_json::json!({"version":1})).unwrap();
        atomic_json(&p, &serde_json::json!({"version":2})).unwrap();
        assert_eq!(
    ...
    #[test]
    fn reserve_accounts_for_high_bitrate() {
        assert!(reserve_bytes(1, 80000) > 1024 * 1024 * 1024);
    }
}
