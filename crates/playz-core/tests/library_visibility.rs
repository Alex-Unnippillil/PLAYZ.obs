...
async fn collections_are_disjoint_and_removal_and_restore_are_idempotent() {
    ...
    let library = Library::open(d.path().join("library.sqlite3")).await.unwrap();
    ...
    assert_eq!(library.list("".into(), 0, 50, false).await.unwrap().total, 2);
    assert_eq!(library.list_removed("".into(), 0, 50, false).await.unwrap().total, 0);
}
...
async fn removal_rejects_active_and_unknown_entries_without_false_success() {
    ...
    assert!(library.remove_entry(uuid::Uuid::new_v4().to_string()).await.is_err());
    assert!(library.restore_entry(uuid::Uuid::new_v4().to_string()).await.is_err());
    assert_eq!(library.list_removed("".into(), 0, 50, false).await.unwrap().total, 0);
    assert_eq!(library.list("".into(), 0, 50, false).await.unwrap().total, 3);
}
...
async fn search_treats_wildcards_literally_in_both_collections() {
    let d = tempfile::tempdir().unwrap();
    let library = Library::open(d.path().join("library.sqlite3")).await.unwrap();
    let special = insert(&library, d.path(), "100% ace_one \\ 勝利 O'Brien", Phase::Ready).await;
    ...
    for removed in [false, true] {
        ...
        for query in ["%", "_", "\\", "勝利", "O'Brien", "ACE_one"] {
            let page = if removed {
                library.list_removed(query.into(), 0, 50, false).await.unwrap()
            } else {
...
async fn removed_pagination_is_stable_bounded_and_searchable_by_notes_and_tags() {
    ...
    for index in 0..55 {
        ...
        library.details(id.clone(), Details {
            title: "Same timestamp".into(), favorite: index == 0,
            tags: vec!["team_ace".into()], notes: "100% preserved".into(),
        }).await.unwrap();
        library.remove_entry(id.clone()).await.unwrap();
        expected.insert(id);
    }
    let first = library.list_removed("".into(), 0, 50, false).await.unwrap();
    let last = library.list_removed("".into(), 50, 50, false).await.unwrap();
    assert_eq!((first.total, last.total), (55, 55));
    assert_eq!((first.items.len(), last.items.len()), (50, 5));
    let actual = first.items.iter().chain(last.items.iter()).map(|r| r.id.clone()).collect::<HashSet<_>>();
    assert_eq!(actual, expected);
    for query in ["team_ace", "100%"] {
        assert_eq!(library.list_removed(query.into(), 0, 50, true).await.unwrap().total, 1);
    }
    assert!(library.list_removed("".into(), 100, 50, false).await.unwrap().items.is_empty());
    for limit in [0, 201] {
        assert!(library.list_removed("".into(), 0, limit, false).await.is_err());
    }
    assert!(library.list_removed("x".repeat(201), 0, 50, false).await.is_err());
}
...
async fn backup_and_reopen_preserve_visibility_edits_bookmarks_paths_and_export_jobs() {
    ...
    library.lifecycle(id.clone(), Phase::Ready, None, Some(4000.0), Some(24), Some(playback.clone())).await.unwrap();
    library.details(id.clone(), Details {
        title: "勝利 O'Brien".into(), favorite: true, tags: vec!["ace".into()], notes: "Keep these notes".into(),
    }).await.unwrap();
    library.resume(id.clone(), 1250.0).await.unwrap();
    let bookmark = Bookmark { id: uuid::Uuid::new_v4().to_string(), recording_id: id.clone(), position_ms: 1000.0, label: "Highlight".into(), note: "Keep bookmark note".into() };
    library.bookmark(bookmark.clone()).await.unwrap();
    let job_id = uuid::Uuid::new_v4().to_string();
    library.enqueue(StoredJob { view: ExportJob {
        id: job_id.clone(), recording_id: id.clone(), state: "completed".into(), mode: ExportMode::Accurate,
        start_ms: 1000.0, end_ms: 3000.0, name: "clip.mp4".into(), progress: 100.0, error: None,
    }, output: output.clone(), temporary: folder.join("clip.tmp.mp4") }).await.unwrap();
    library.job_state(job_id.clone(), "completed", 100.0, None).await.unwrap();
    let before = library.get(id.clone()).await.unwrap();
    library.remove_entry(id.clone()).await.unwrap();
    let backup = library.backup(d.path().join("backup.sqlite3")).await.unwrap();
    drop(library);
    for database in [path, backup] {
        let reopened = Library::open(database).await.unwrap();
        reopened.reconcile_states().await.unwrap();
        assert_eq!(reopened.list("".into(), 0, 50, false).await.unwrap().total, 0);
        assert_eq!(reopened.list_removed("ace".into(), 0, 50, true).await.unwrap().total, 1);
        reopened.restore_entry(id.clone()).await.unwrap();
        let after = reopened.get(id.clone()).await.unwrap();
        assert_eq!(serde_json::to_value(&before.view).unwrap(), serde_json::to_value(&after.view).unwrap());
        ...
        assert_eq!(reopened.list("ace".into(), 0, 50, true).await.unwrap().total, 1);
    }
    assert_eq!(fs::read(&before.master).unwrap(), b"preserved master fixture");
